...
export async function onMessageReactionAdd(reaction: MessageReaction, user: User | PartialUser): Promise<void> {
    try {
        if (reaction.partial) {
    ...
    if (reaction.emoji.name === "⭐") {
        Starboard.addStar(reaction.message.guild as Guild, user, reaction, gData).catch(e => { });
        return;
    }
